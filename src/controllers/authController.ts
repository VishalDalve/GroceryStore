...
class AuthController {
    ...
    public async login(req: express.Request, res: express.Response, next: express.NextFunction): Promise<any> {
        ...
        try{
            ...
            // let user = await UserModel.findOne({'email': req.body.email, 'isVerified': true, 'isActive': true})
            ...
            //console.log("userLog2 :",user);
             ...
             if (!user) {
                return res.status(404).send({"message":Messages.ERROR_USER});
                
            }
           
           if(!user.isVerified)
            {
              return res.status(400).send({"message":"Please check your email id for verification link"});
               
            }
            
            if(!user.isActive)
            {
                return res.status(400).send({"message":"User Deactivated by Admin"});
    ...
    public async signUp(req: express.Request, res: express.Response, next: express.NextFunction): Promise<any> {
       try {
            ...
            Mailer.sendMail(value.email, "EMAIL VERIFICATION", {"email":value.email,"name": user.name, "url": config.envConfig.EMAIL_VERIFICATION_URL + '/' + emailToken})
    ...
    public async forgotPassword(req: express.Request, res: express.Response, next: express.NextFunction): Promise<any> {
        ...
        try{
            ...
            if (!user) {
                res.status(404).send({"message":Messages.ERROR_USER})
            }

            let token = AuthService.passwordToken(user._id);

            await Mailer.sendMail(user.email, "Reset password", {"email":user.email,"url" : config.envConfig.RESET_PASSWORD_URL + '?' + token})
            return res.status(200).json({"message": Messages.PASSWORD_RESET, "data": token})
